...
BACKUP="${1:-}"
if [[ -z "$BACKUP" ]]; then BACKUP="$(ls -1t "$INSTALL_DIR"/backups/deebot-x1-local-*.tar.gz 2>/dev/null | head -1 || true)"; fi
...
[[ -d "$BUMPER" ]] && (cd "$BUMPER" && docker compose down) || true
[[ -f "$INSTALL_DIR/.dashboard.env" ]] && (cd "$INSTALL_DIR" && docker compose -f docker-compose.dashboard.yml down) || true
